...
- [ ] Phase 5: AI生成（内部）
- [x] Phase 6: MCP公開・APIキー管理（`api_keys`のCRUD API（`GET/POST /api-keys`, `DELETE /api-keys/:id`、いずれも`requireAuth`によるセッション認証必須）を実装。生キーは発行時（`POST /api-keys`）のレスポンスでのみ返し、DBにはSHA-256ハッシュのみ保存。`apps/kizashi-mcp`に`POST /mcp`（ステートレスなStreamable HTTP、`Authorization: Bearer <APIキー>`で認証）を実装し、設計書「5. MCPツール設計」の6ツール（`list_drafts`/`get_draft`/`get_group_stats`/`list_projects`/`get_project_file`/`create_draft`）を`packages/kizashi-core`のロジック経由で公開。スコープ検証はツール単位（キーが無効→HTTP 401、スコープ不足→`tools/call`結果を`isError:true`で返す）。ローカルで`wrangler dev`を2 Worker同時起動しサインアップ→APIキー発行→MCP経由のtools/list・tools/call（scope拒否・not_found含む）まで動作確認済み）
- [ ] Phase 7: 複数アカウントUI仕上げ（`docs/design-ui.md`のデザイントークンをshadcn/uiテーマへ正式統合する作業を含む）。ヘッダーのアカウントスイッチャー（`AccountSwitcher`、選択状態は`account_id`クエリ＋`localStorage`で保持しDraft一覧をフィルタ、「すべてのアカウント」で全アカウント横断ビュー）、`/onboarding`画面（サインアップ直後の遷移先。Threads OAuth連携ボタン→連携済みアカウント一覧表示）を実装済み。`threads-accounts/oauth/callback`はブラウザ遷移に対応するため、これまでのJSON応答から`${FRONTEND_ORIGIN}/onboarding?connected=1`（失敗時は`?error=1`）への302リダイレクトに変更。shadcn/uiテーマへの正式統合自体は未着手
...
- `main`ブランチへのpushで`.github/workflows/deploy.yml`が自動実行され、D1マイグレーション適用（remote）→`kizashi-api`→`kizashi-mcp`→`kizashi-web`の順にCloudflareへデプロイする。手動実行は`workflow_dispatch`からも可能。必要なRepository Secretsは`CLOUDFLARE_API_TOKEN`（Workers Scripts編集・D1編集権限を持つトークン）と`CLOUDFLARE_ACCOUNT_ID`。各WorkerのWorkers Secrets（`TOKEN_ENCRYPTION_KEY`等）はこのワークフローでは管理せず、従来通り`wrangler secret put`で個別設定する。Repository Secrets設定後、run 3（`49c0151`）で自動デプロイの疎通確認済み。ハマりどころ2点：(1) `apps/kizashi-web`のtypecheckは`next build`を経ずに`tsc --noEmit`だけ実行すると`.next/types`配下の型（`LayoutProps`等）が無くTS2304になるため、`typecheck`スクリプトを`next typegen && tsc --noEmit`にしている。(2) `pnpm --filter <pkg> deploy`はpackage.jsonの`deploy`スクリプトではなくpnpmの予約コマンド（workspaceデプロイ）として解釈され`ERR_PNPM_INVALID_DEPLOY_TARGET`になるため、ワークフロー内では`pnpm --filter <pkg> run deploy`と`run`を明示している
